package com.arpokrat.common.ui.theme
...
object ThemeManager {
  ...
  private fun systemDarkThemeColors(): Pair<Colors, DefaultTheme> = when (appPrefs.systemDarkTheme.get()) {
    ...
    else -> BlackColorPalette to DefaultTheme.BLACK
  }
  ...
  private fun nonSystemThemeName(): String {
    ...
    return if (themeName != DefaultTheme.SYSTEM_THEME_NAME) {
      ...
      if (systemInDarkThemeCurrently) appPrefs.systemDarkTheme.get()!! else DefaultTheme.BLACK.themeName
    }
  }
  ...
  fun defaultActiveTheme(perUserTheme: ThemeModeOverrides?, appSettingsTheme: List<ThemeOverrides>): ThemeModeOverride {
    ...
    return ThemeModeOverride(colors = defaultTheme?.colors ?: ThemeColors(), wallpaper = defaultTheme?.wallpaper
      ?: ThemeWallpaper.from(PresetWallpaper.ARPOKRAT.toType(CurrentColors.value.base), null, null))
  }
  ...
  fun currentColors(themeOverridesForType: WallpaperType?, perChatTheme: ThemeModeOverride?, perUserTheme: ThemeModeOverrides?, appSettingsTheme: List<ThemeOverrides>): ActiveTheme {
    ...
    val baseTheme = when (nonSystemThemeName) {
      DefaultTheme.LIGHT.themeName -> ActiveTheme(DefaultTheme.LIGHT.themeName, DefaultTheme.LIGHT, LightColorPalette, LightColorPaletteApp, AppWallpaper(type = PresetWallpaper.ARPOKRAT.toType(DefaultTheme.LIGHT)))
      DefaultTheme.DARK.themeName -> ActiveTheme(DefaultTheme.DARK.themeName, DefaultTheme.DARK, DarkColorPalette, DarkColorPaletteApp, AppWallpaper(type = PresetWallpaper.ARPOKRAT.toType(DefaultTheme.DARK)))
      DefaultTheme.SIMPLEX.themeName -> ActiveTheme(DefaultTheme.SIMPLEX.themeName, DefaultTheme.SIMPLEX, SimplexColorPalette, SimplexColorPaletteApp, AppWallpaper(type = PresetWallpaper.ARPOKRAT.toType(DefaultTheme.SIMPLEX)))
      ...
      else -> ActiveTheme(DefaultTheme.BLACK.themeName, DefaultTheme.BLACK, BlackColorPalette, BlackColorPaletteApp, AppWallpaper(type = WallpaperType.Empty))
    }
  ...
  fun Color.toReadableHex(): String {
    ...
    return when {
      ...
      s.length == 2 -> "#ff$s$s$s"
      s.length == 3 -> "#ff$s$s"
      s.length == 4 && this.alpha == 0f -> "#0000$s"
      s.length == 4 -> "#ff00$s"
