...
import com.arpokrat.common.model.MsgFilter.*
...
import com.arpokrat.common.views.chat.item.contentModerationPostLink
import com.arpokrat.common.views.chatlist.*
import com.arpokrat.common.views.helpers.*
import com.arpokrat.common.views.migration.MigrationToDeviceState
    ...
    suspend fun addChatItem(rhId: Long?, chatInfo: ChatInfo, cItem: ChatItem) {
      ...
      withContext(Dispatchers.Main) {
        if (chatItemBelongsToScope(cInfo, cItem)) {
          val newKey = cItem.meta.itemId ?: cItem.id
          if (chatItems.value.none { (it.meta.itemId ?: it.id) == newKey }) {
    ...
    fun removeChatItem(rhId: Long?, cInfo: ChatInfo, cItem: ChatItem) {
      if (cInfo.groupChatScope() == null) {
        ...
        if (i >= 0) {
          ...
          val pItem = chat.chatItems.lastOrNull()
          if (pItem?.id == cItem.id) {
            val newItems = chat.chatItems.filter { it.id != cItem.id }
            chats[i] = chat.copy(chatItems = newItems)
            if (newItems.isEmpty() || newItems.all { it.meta.itemDeleted != null }) {
              CoroutineScope(Dispatchers.IO).launch {
                try {
                  val result = controller.apiGetChat(
                    rhId,
                    cInfo.chatType,
                    cInfo.apiId,
                    cInfo.groupChatScope(),
                    null,
                    ChatPagination.Last(1),
                    ""
                  )
                  if (result != null) {
                    val (updatedChat, _) = result
                    withContext(Dispatchers.Main) {
                      val currentIndex = getChatIndex(rhId, cInfo.id)
                      if (currentIndex >= 0) {
                        chats[currentIndex] = updatedChat
                      }
                    }
                  }
                } catch (e: Exception) {
...
data class Connection(
  ...
  val connDisabled: Boolean
    get() = authErrCounter >= 10

  val connInactive: Boolean
    get() = quotaErrCounter >= 5
...
data class ChatItem (
  ...
  val mergeCategory: CIMergeCategory?
    get() = when (content) {
      ...
      is CIContent.SndChatFeature -> null
  ...
  companion object {
    ...
    fun invalidJSON(): CIMeta =
      CIMeta(
        ...
        itemTs = Clock.System.now(),
        itemText = generalGetString(MR.strings.invalid_json_format),
...
sealed class CIStatus {
  ...
  fun statusIcon(
    ...
    when (this) {
      ...
      is SndRcvd -> when(this.msgRcptStatus) {
        ...
        MsgReceiptStatus.BadMsgHash -> MR.images.ic_double_check to CurrentColors.value.colors.error
      }
      is SndErrorAuth -> MR.images.ic_close to CurrentColors.value.colors.error
      is CISSndError -> MR.images.ic_close to CurrentColors.value.colors.error
  ...
  val statusInto: Pair<String, String>? get() = when (this) {
    ...
    is RcvNew -> null
    is RcvRead -> null
    is Invalid -> generalGetString(MR.strings.invalid_status) to this.text
  }
}
...
sealed class GroupSndStatus {
  ...
  fun statusIcon(
    ...
    when (this) {
      ...
      is Error -> MR.images.ic_close to CurrentColors.value.colors.error
...
sealed class CIContent: ItemContent {
  ...
  override val text: String get() = when (this) {
    ...
    is ChatBanner -> ""
    is InvalidJSON -> generalGetString(MR.strings.invalid_data)
  }
    ...
    return if (json is JsonObject) {
      if ("type" in json) {
        ...
        when (t) {
          ...
          "image" -> {
            val image = json["image"]?.jsonPrimitive?.content ?: generalGetString(MR.strings.unknown_message_format)
...
enum class FormatColor(val color: String) {
  ...
  val uiColor: Color @Composable get() = when (this) {
    red -> CurrentColors.value.colors.error
    green -> SimplexGreen
    blue -> SimplexBlue
...
sealed class MsgErrorType() {
  ...
  val text: String get() = when (this) {
    ...
    is MsgBadHash -> generalGetString(MR.strings.integrity_msg_bad_hash)
    is MsgBadId -> generalGetString(MR.strings.integrity_msg_bad_id)
    is MsgDuplicate -> generalGetString(MR.strings.integrity_msg_duplicate)
  }
}
