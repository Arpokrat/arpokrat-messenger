...
import com.arpokrat.common.views.usersettings.showInDevelopingAlert
...
fun SendMsgView(
  ...
  val padding = if (appPlatform.isAndroid) PaddingValues(vertical = 8.dp, horizontal = 4.dp) else PaddingValues(top = 6.dp, bottom = 6.dp)
  ...
  Box(Modifier.padding(padding)) {
    ...
    Box(Modifier.align(Alignment.BottomEnd).padding(bottom = if (appPlatform.isAndroid) 2.dp else 5.sp.toDp() * fontSizeSqrtMultiplier)) {
      val sendButtonSize = remember { Animatable(42f) }
      ...
      when {
        ...
        cs.liveMessage?.sent == false && cs.message.text.isEmpty() -> {
          ...
          val currentComposeState = composeState.value
          val icon = if (currentComposeState.editing || currentComposeState.liveMessage != null) painterResource(MR.images.ic_check_filled) else painterResource(MR.images.ic_arrow_upward)
          ...
          fun MenuItems(): List<@Composable () -> Unit> {
            ...
            if (currentComposeState.liveMessage == null && !currentComposeState.editing && !nextConnect || sendMsgEnabled) {
              if (
                currentComposeState.preview !is ComposePreview.VoicePreview &&
                currentComposeState.contextItem is ComposeContextItem.NoContextItem &&
...
private fun DisallowedVoiceButton(onClick: () -> Unit) {
  IconButton(onClick, Modifier.size(42.dp)) {
    Icon(
...
private fun RecordVoiceButton(interactionSource: MutableInteractionSource) {
  IconButton({}, Modifier.size(42.dp), interactionSource = interactionSource) {
    Icon(
      painterResource(MR.images.ic_keyboard_voice_filled),
      stringResource(MR.strings.icon_descr_record_voice_message),
      tint = MaterialTheme.colors.primary,
...
fun ComposeProgressIndicator() {
  CircularProgressIndicator(Modifier.size(42.dp).padding(4.dp), color = MaterialTheme.colors.secondary, strokeWidth = 2.5.dp)
}
  ...
  Box(
    ...
    Icon(
      ...
      modifier = Modifier
        ...
        .alpha(alpha.value)
        .clip(CircleShape)
        .background(if (enabled) sendButtonColor else MaterialTheme.colors.secondary.copy(alpha = 0.5f))
        .padding(6.dp)
    )
  }
}
  ...
  fun run() {
    scope.launch {
      while (composeState.value.liveMessage != null) {
        sendButtonSize.animateTo(if (sendButtonSize.value == 42f) 38f else 42f, tween(700, 50))
      }
      sendButtonSize.snapTo(42f)
    }
    scope.launch {
      while (composeState.value.liveMessage != null) {
