...
import com.arpokrat.common.model.ChatController.getServerOperators
import com.arpokrat.common.model.ChatController.getUserServers
import com.arpokrat.common.model.ChatController.setUserServers
...
import com.arpokrat.common.views.onboarding.OnboardingActionButton
import com.arpokrat.common.views.onboarding.OnboardingInformationButton
...
fun ModalData.NetworkAndServersView(closeNetworkAndServers: () -> Unit) {
  ...
  fun onClose(close: () -> Unit): Boolean {
    val canSaveServers = serversCanBeSaved(currUserServers.value, userServers.value, serverErrors.value)
    if (!canSaveServers) {
  ...
  ColumnWithScrollBar {
    ...
    if (!chatModel.desktopNoUserNoRemote) {
      SectionView(generalGetString(MR.strings.network_preset_servers_title).uppercase()) {
        val serverOperators = m.conditions.value.serverOperators
        if (serverOperators.isNotEmpty()) {
          serverOperators.forEachIndexed { index, operator ->
            val uServers = userServers.value.firstOrNull { it.operator?.operatorId == operator.operatorId }
            if (uServers != null) {
              val sIndex = userServers.value.indexOf(uServers)
              ServerOperatorRow(
                index = sIndex,
                operator = operator,
                currUserServers = currUserServers,
              ...
              val uServersNil = userServers.value.firstOrNull { it.operator == null }
              val indexAdjust = if (uServersNil != null) 1 else 0
              ServerOperatorRow(
                index = index + indexAdjust,
                operator = operator,
                currUserServers = currUserServers,
                userServers = userServers,
                serverErrors = serverErrors,
...
private fun ServerOperatorRow(
  ...
  val isAvailable = operator.operatorTag == OperatorTag.SWITZERLAND
  ...
  SectionItemView(
    ...
    val flag = when (operator.operatorTag) {
      OperatorTag.SWITZERLAND -> "🇨🇭"
      OperatorTag.ICELAND -> "🇮🇸"
      OperatorTag.PANAMA -> "🇵🇦"
      OperatorTag.MALAYSIA -> "🇲🇾"
      OperatorTag.SOUTH_AFRICA -> "🇿🇦"
      else -> "🌐"
    }
    ...
    Text(
      text = flag,
      fontSize = 24.sp,
      modifier = Modifier.alpha(if (isAvailable && operator.enabled) 1f else 0.5f)
    )
    TextIconSpaced()
    Text(
      text = operator.tradeName,
      color = if (isAvailable && operator.enabled) MaterialTheme.colors.onBackground else MaterialTheme.colors.secondary
    )
    ...
    if (!isAvailable) {
      Box(
        ...
        Text(
          text = stringResource(MR.strings.in_developing_title),
          fontSize = 10.sp,
    ...
    val isActive = isAvailable && operator.enabled
    Box(
      modifier = Modifier
        .background(
          color = if (isActive) Color(0xFF10B981).copy(alpha = 0.15f) else MaterialTheme.colors.onSurface.copy(alpha = 0.1f),
          shape = RoundedCornerShape(12.dp)
        )
        .padding(horizontal = 8.dp, vertical = 4.dp),
      contentAlignment = Alignment.Center
      ...
      Row(verticalAlignment = Alignment.CenterVertically) {
        Box(
          modifier = Modifier
            .size(6.dp)
            .background(
              color = if (isActive) Color(0xFF10B981) else MaterialTheme.colors.onSurface.copy(alpha = 0.4f),
              shape = CircleShape
            )
        )
        Spacer(Modifier.width(6.dp))
        Text(
          text = if (isActive) stringResource(MR.strings.status_active) else stringResource(MR.strings.status_inactive),
          fontSize = 12.sp,
          fontWeight = FontWeight.Bold,
          color = if (isActive) Color(0xFF10B981) else MaterialTheme.colors.onSurface.copy(alpha = 0.4f)
        )
