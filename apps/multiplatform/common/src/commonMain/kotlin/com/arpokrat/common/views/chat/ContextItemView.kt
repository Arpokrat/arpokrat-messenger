...
import com.arpokrat.common.platform.getLoadedFilePath
...
fun ContextItemView(
  ...
  fun MessageText(contextItem: ChatItem, attachment: ImageResource?, lines: Int) {
    ...
    val rawText = contextItem.text
    val (displayText, displayFormatted) = if (CryptoProtocol.isInvoice(rawText)) {
      val invoice = CryptoProtocol.parseInvoice(rawText)
      if (invoice != null) {
        ...
        rawText to contextItem.formattedText
      }
    } else if (CryptoProtocol.isPaid(rawText)) {
      val text = generalGetString(MR.strings.crypto_invoice_paid_preview)
      text to FormattedText.plain(text)
    } else if (CryptoProtocol.isCancelled(rawText)) {
      val text = generalGetString(MR.strings.crypto_invoice_cancelled_preview)
      text to FormattedText.plain(text)
    } else if (CryptoProtocol.isDeclined(rawText)) {
