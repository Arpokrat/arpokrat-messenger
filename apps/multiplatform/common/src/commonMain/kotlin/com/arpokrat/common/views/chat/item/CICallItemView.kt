  ...
  Column(
    ...
    when (status) {
      CICallStatus.Pending -> if (sent) {
        Icon(painterResource(MR.images.ic_call), stringResource(MR.strings.icon_descr_call_pending_sent), tint = MaterialTheme.colors.secondary)
      } else {
        AcceptCallButton(cInfo, acceptCall)
      }
      CICallStatus.Missed -> Icon(painterResource(MR.images.ic_call), stringResource(MR.strings.icon_descr_call_missed), tint = MaterialTheme.colors.error)
      CICallStatus.Rejected -> Icon(painterResource(MR.images.ic_call_end), stringResource(MR.strings.icon_descr_call_rejected), tint = MaterialTheme.colors.error)
      ...
      CICallStatus.Error -> Icon(painterResource(MR.images.ic_warning_filled), null, tint = MaterialTheme.colors.error)
    }
...
fun AcceptCallButton(cInfo: ChatInfo, acceptCall: (Contact) -> Unit) {
  if (cInfo is ChatInfo.Direct) {
    Button(
      onClick = { acceptCall(cInfo.contact) },
      shape = CircleShape,
      ...
      Icon(
        painterResource(MR.images.ic_ring_volume),
        contentDescription = null,
        modifier = Modifier.size(20.dp).padding(end = 8.dp)
      )
      Text(stringResource(MR.strings.answer_call))
    }
  } else {
