...
import com.arpokrat.common.views.usersettings.PreferenceToggle
...
private fun FeatureSection(
  ...
  val statusColor = if (enabled.forUser) SimplexGreen else if (enabled.forContact) WarningYellow else MaterialTheme.colors.error
  ...
  SectionView(
    ...
    PreferenceToggle(
      ...
      checked = allowFeature.value.allowed == FeatureAllowed.YES,
    ) { allow ->
      onSelected(if (allow) ContactFeatureAllowed.Yes else ContactFeatureAllowed.No)
    }
...
private fun TimedMessagesFeatureSection(
  ...
  SectionView(
    ...
    iconTint = if (enabled.forUser) SimplexGreen else if (enabled.forContact) WarningYellow else MaterialTheme.colors.error,
    leadingIcon = true,
  ) {
    PreferenceToggle(
      generalGetString(MR.strings.chat_preferences_you_allow),
