package com.arpokrat.common.views.remote
...
import com.arpokrat.common.views.newchat.QRCode
import com.arpokrat.common.views.usersettings.*
import com.arpokrat.common.views.usersettings.networkAndServers.validPort
      ...
      for (host in remoteHosts) {
        ...
        Box(Modifier.padding(horizontal = DEFAULT_PADDING)) {
          DefaultDropdownMenu(showMenu) {
            if (host.activeHost) {
              ItemAction(stringResource(MR.strings.disconnect_remote_host), painterResource(MR.images.ic_wifi_off), color = MaterialTheme.colors.error) {
                stopRemoteHostAndReloadHosts(host, true)
                showMenu.value = false
              }
            } else {
              ItemAction(stringResource(MR.strings.delete_verb), painterResource(MR.images.ic_delete), color = MaterialTheme.colors.error) {
                deleteHost(host)
                showMenu.value = false
              }
            }
...
fun DeviceNameField(
  ...
  DefaultConfigurableTextField( // TODO Replace by ModernTextField
    state = state,
  ...
  ModalManager.start.showModalCloseable { close ->
    ...
    Column {
      ...
      SectionItemView(disconnectHost) {
        Text(generalGetString(MR.strings.disconnect_remote_host), Modifier.fillMaxWidth(), color = MaterialTheme.colors.error)
      }
    }
