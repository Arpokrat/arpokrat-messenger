...
import com.arpokrat.common.ui.theme.ThemeManager.colorFromReadableHex
...
enum class PresetWallpaper(
  ...
  ARPOKRAT(MR.images.wallpaper_arpokrat, "arpokrat", 1f,

    background = mapOf(
      DefaultTheme.LIGHT to "#ffffffff".colorFromReadableHex(),
      DefaultTheme.DARK to "#ff000000".colorFromReadableHex(),
      DefaultTheme.SIMPLEX to "#ff000000".colorFromReadableHex(),
      DefaultTheme.BLACK to "#ff000000".colorFromReadableHex()
    ),
    tint = mapOf(
      DefaultTheme.LIGHT to Color.Transparent,
      DefaultTheme.DARK to Color.Transparent,
      DefaultTheme.SIMPLEX to Color.Transparent,
      DefaultTheme.BLACK to Color.Transparent
    ),

    mapOf(
      DefaultTheme.LIGHT to ThemeColors(
        sentMessage = "#ffF2F2F2",
        sentQuote = "#ffE5E5E5",
        receivedMessage = "#ffffffff",
        receivedQuote = "#ffF5F5F5",
      ),
      DefaultTheme.DARK to ThemeColors(
        sentMessage = "#ff2C2C2C",
        sentQuote = "#ff383838",
        receivedMessage = "#ff121212",
        receivedQuote = "#ff1E1E1E",
      ),
      ...
      DefaultTheme.BLACK to ThemeColors(
        sentMessage = "#FF2C2C2E",
        sentQuote = "#FF3A3A3C",
        receivedMessage = "#FF151515",
        receivedQuote = "#FF252525",
      ),
    )
  ),
...
fun CacheDrawScope.chatViewBackground(
  ...
  val useBitmapDrawing = (imageType is WallpaperType.Preset && tint != Color.Transparent) ||
      (imageType is WallpaperType.Image && imageType.scaleType == WallpaperScaleType.REPEAT)

  val imageToDraw = if (useBitmapDrawing) {
      ...
      when (imageType) {
        is WallpaperType.Preset -> {
          if (tint == Color.Transparent) {
            val scaleType = WallpaperScaleType.FILL
            clipRect {
              ...
              drawImage(
                imageToDraw,
                dstOffset = IntOffset(
                  x = ((size.width - scaledWidth) / 2).roundToInt(),
                  y = ((size.height - scaledHeight) / 2).roundToInt()
                ),
        ...
        is WallpaperType.Image -> when (val scaleType = imageType.scaleType ?: WallpaperScaleType.FILL) {
          WallpaperScaleType.REPEAT -> drawImage(imageToDraw)
          WallpaperScaleType.FILL, WallpaperScaleType.FIT -> {
            clipRect {
              ...
              drawImage(imageToDraw, dstOffset = IntOffset(x = ((size.width - scaledWidth) / 2).roundToInt(), y = ((size.height - scaledHeight) / 2).roundToInt()), dstSize = IntSize(scaledWidth, scaledHeight), filterQuality = quality)
              if (scaleType == WallpaperScaleType.FIT) {
                if (scaledWidth < size.width) {
                  var x = (size.width - scaledWidth) / 2
                  while (x > 0) {
                    drawImage(imageToDraw, dstOffset = IntOffset(x = (x - scaledWidth).roundToInt(), y = ((size.height - scaledHeight) / 2).roundToInt()), dstSize = IntSize(scaledWidth, scaledHeight), filterQuality = quality)
                    x -= scaledWidth
                  }
                  x = size.width - (size.width - scaledWidth) / 2
                  while (x < size.width) {
                    drawImage(imageToDraw, dstOffset = IntOffset(x = x.roundToInt(), y = ((size.height - scaledHeight) / 2).roundToInt()), dstSize = IntSize(scaledWidth, scaledHeight), filterQuality = quality)
                    x += scaledWidth
                  }
                } else {
                  var y = (size.height - scaledHeight) / 2
                  while (y > 0) {
                    drawImage(imageToDraw, dstOffset = IntOffset(x = ((size.width - scaledWidth) / 2).roundToInt(), y = (y - scaledHeight).roundToInt()), dstSize = IntSize(scaledWidth, scaledHeight), filterQuality = quality)
                    y -= scaledHeight
                  }
                  y = size.height - (size.height - scaledHeight) / 2
                  while (y < size.height) {
                    drawImage(imageToDraw, dstOffset = IntOffset(x = ((size.width - scaledWidth) / 2).roundToInt(), y = y.roundToInt()), dstSize = IntSize(scaledWidth, scaledHeight), filterQuality = quality)
                    y += scaledHeight
                  }
                }
              }
