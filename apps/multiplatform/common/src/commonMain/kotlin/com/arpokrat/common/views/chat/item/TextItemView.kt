...
import com.arpokrat.common.ui.theme.CurrentColors
import com.arpokrat.common.views.helpers.*
import com.arpokrat.res.*
            ...
            is Format.Mention -> {
              ...
              if (mention != null) {
                ...
                if (mention.memberRef != null) {
                  ...
                  withAnnotation(tag = "MENTION", annotation = mention.memberId) {
                    withStyle(mentionStyle) { append(mentionText(name)) }
                  }
                } else {
                  withStyle(mentionStyle) { append(mentionText(ft.format.memberName)) }
                }
              } else {
                append(ft.text)
              }
...
fun safeOpenUri(uri: String, uriHandler: UriHandler) {
  try {
    ...
    showInvalidLinkAlert(uri, error = generalGetString(MR.strings.error))
  }
}
