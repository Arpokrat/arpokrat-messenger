...
import com.arpokrat.common.platform.appPlatform
...
fun getArpokratJurisdictionIceServers(activeJurisdictions: String): List<RTCIceServer> {
  val servers = ArrayList<RTCIceServer>()

  val turnUser = BuildConfigCommon.TURN_USERNAME
  val turnPass = BuildConfigCommon.TURN_PASSWORD

  // Switzerland
  if (activeJurisdictions.contains("Switzerland", ignoreCase = true)) {
    servers.add(RTCIceServer(listOf("stun:turn1-ch.arpokrat.com:3478")))
    servers.add(RTCIceServer(
      listOf("turns:turn1-ch.arpokrat.com:443?transport=tcp"),
      turnUser,
      turnPass
    ))
  }
  ...
  // Iceland
  if (activeJurisdictions.contains("Iceland", ignoreCase = true)) {
  }

  // Panama
  if (activeJurisdictions.contains("Panama", ignoreCase = true)) {
  }

  // South Africa
  if (activeJurisdictions.contains("South Africa", ignoreCase = true)) {
  }

  // Malaysia
  if (activeJurisdictions.contains("Malaysia", ignoreCase = true)) {
  }
...
fun getIceServers(activeJurisdictions: String): List<RTCIceServer>? {

  val value = ChatController.appPrefs.webrtcIceServers.get()
  if (value != null && value.isNotEmpty()) {
    val servers: List<String> = value.split("\n")
    val parsedServers = parseRTCIceServers(servers)
    if (parsedServers != null) return parsedServers
  }

  return getArpokratJurisdictionIceServers(activeJurisdictions)
}