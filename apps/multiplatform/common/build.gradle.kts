  ...
  sourceSets {
    ...
    val androidMain by getting {
      ...
      dependencies {
        ...
        // Barcode
        implementation("org.boofcv:boofcv-android:1.1.5")
        ...
        implementation("com.trustwallet:wallet-core-kotlin-android:4.5.0")
        implementation("com.trustwallet:wallet-core-proto:4.5.0")
...
multiplatformResources {
  multiplatformResourcesPackage = "com.arpokrat.res"
...
val etherscanKey = localProps.getProperty("etherscan.api.key") ?: ""
val alchemyKey = localProps.getProperty("alchemy.api.key") ?: ""
val turnUsername = localProps.getProperty("turn.username") ?: ""
val turnPassword = localProps.getProperty("turn.password") ?: ""
...
afterEvaluate {
  ...
  tasks.create("adjustFormatting") {
    doLast {
      ...
      val anyHtmlRegex = Regex("[^>]*>.*(<b>|<i>|<u>|<font|&lt;b&gt;|&lt;i&gt;|&lt;u&gt;|&lt;font).*</string>")
      ...
      fun String.formatting(filepath: String): List<String> {
        if (contains("""formatted="false"""")) return emptyList()
        ...
        while (i < value.length) {
          ...
          // Fixing issue with % in MR String
          if (i + 1 < value.length && value[i + 1] == '%') {
            i += 2
            continue
          }

          val remaining = value.substring(i + 1)

          val matchedFormat = possibleFormat.firstOrNull { remaining.startsWith(it) }
          if (matchedFormat != null) {
            formats.add(matchedFormat)
            i += 1 + matchedFormat.length
            continue
          }

          throw Exception(
            "Unknown formatting in string. Add it to 'possibleFormat' in common/build.gradle.kts if needed: $this \nin $filepath"
          )
        }

        return if (formats.any { it.startsWith("1$") || it.startsWith("2$") || it.startsWith("3$") || it.startsWith("4$") }) {
          formats.sortedBy {
            it.trim('s', 'd', 'f', '$').toIntOrNull()
              ?: throw Exception("Formatting don't have positional arguments: $this \nin $filepath")
          }
        } else {
          formats
        }
      ...
      fun String.addCDATA(filepath: String): String {
        ...
        if (anyHtmlRegex.matches(this)) {
          ...
          val cleanForCount = prepared.replace(Regex("\\s+>\\s+|->|=>|&gt;"), "")

          val countOfStartTag = cleanForCount.count { it == '<' }
          val countOfEndTag = cleanForCount.count { it == '>' }
          if (countOfStartTag != countOfEndTag || countOfStartTag != endTagRegex.findAll(cleanForCount).count() * 2 || !correctHtmlRegex.matches(prepared)) {
            if (debug) {
