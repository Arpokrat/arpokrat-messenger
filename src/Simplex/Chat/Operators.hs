...
data OperatorTag = OTSwitzerland | OTIceland | OTPanama | OTMalaysia | OTSouthAfrica
...
instance TextEncoding OperatorTag where
  textDecode = \case
    "switzerland" -> Just OTSwitzerland
    "iceland" -> Just OTIceland
    "panama" -> Just OTPanama
    "malaysia" -> Just OTMalaysia
    "southafrica" -> Just OTSouthAfrica
    _ -> Nothing
  textEncode = \case
    OTSwitzerland -> "switzerland"
    OTIceland -> "iceland"
    OTPanama -> "panama"
    OTMalaysia -> "malaysia"
    OTSouthAfrica -> "southafrica"
...
usageConditionsAction _ _ _ = Nothing
{-  
-- We bypass operator specific condition checks, as Arpokrat handles them globally during onboarding.
